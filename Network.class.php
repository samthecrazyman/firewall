...
class Network {
		...
		if (is_readable("/etc/network/interfaces.d/$int")) {
			...
			foreach ($lines as $line) {
				if (trim($line) == "auto $int") {
					$onBoot = 'yes';
				} else if (strpos($line, "# zone ") !== false) {
					$data = explode('# zone ', trim($line));
					$conf['ZONE'] = $data[1];
				} else if (strpos($line, "# description ") !== false) {
					$data = explode('# description ', trim($line));
					$conf['DESCRIPTION'] = $data[1];
	...
	public function updateInterfaceZone($iface, $newzone = false, $descr = false) {
		...
		$srcfile = "/etc/network/interfaces.d/$iface";
